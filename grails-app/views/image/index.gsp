		...
		<div id="list-image" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="src" title="${message(code: 'image.src.label', default: 'Src')}" />
						<th>Aperçu image</th>
					</tr>
				</thead>
				<tbody>
				<g:each in="${imageInstanceList}" status="i" var="imageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						<td><g:link action="show" id="${imageInstance.id}">${fieldValue(bean: imageInstance, field: "src")}</g:link></td>
						<td><asset:image src='${fieldValue(bean: imageInstance, field: "src")}'  width="200" height="200"/> </td>
					</tr>
				</g:each>
				</tbody>
			</table>
