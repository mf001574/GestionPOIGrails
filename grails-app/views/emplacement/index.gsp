		...
		<div id="list-emplacement" class="content scaffold-list" role="main">
			...
			<thead>
					<tr>
						...
						<g:sortableColumn property="latitude" title="${message(code: 'emplacement.latitude.label', default: 'Latitude')}" />
					
						<g:sortableColumn property="longitude" title="${message(code: 'emplacement.longitude.label', default: 'Longitude')}" />
				...
				<g:each in="${emplacementInstanceList}" status="i" var="emplacementInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td>${fieldValue(bean: emplacementInstance, field: "latitude")}</td>
					
						<td>${fieldValue(bean: emplacementInstance, field: "longitude")}</td>
